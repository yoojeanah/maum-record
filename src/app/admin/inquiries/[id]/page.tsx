...
async function InquiryDetailPage({ params }: { params: Params }) {
  ...
  return (
    <div className="p-6">
      ...
      <div className="bg-white shadow p-4 rounded space-y-3">
        <div>
          <strong>제목:</strong> {mockInquiry.title}
        </div>
        <div>
          <strong>이메일:</strong> {mockInquiry.email}
        </div>
        <div>
          <strong>등록일:</strong>{" "}
          {new Date(mockInquiry.createdAt).toLocaleString()}
        </div>
        <div>
          <strong>문의 내용:</strong>
